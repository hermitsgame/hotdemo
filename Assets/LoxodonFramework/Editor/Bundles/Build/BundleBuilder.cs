...
*/
				// add by xiong below
				var bundleManifests = this.CreateBundleManifests(genOutput, manifest, version);

				foreach (var item in bundleManifests)
				{
					var bm = item.Value;
					var dir = versionOutput + "/" + item.Key;
					bm = this.CopyAssetBundle(bm, new DirectoryInfo(genOutput), new DirectoryInfo(dir), bundleModifierChain);
					File.WriteAllText(string.Format("{0}/{1}/{2}", versionOutput, item.Key, BundleSetting.ManifestFilename), bm.ToJson());
				}
            }
            finally
            {
