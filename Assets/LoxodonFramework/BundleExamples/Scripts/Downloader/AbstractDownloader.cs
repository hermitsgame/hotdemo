...
namespace Loxodon.Framework.Examples.Bundle
{
    public abstract class AbstractDownloader : IDownloader
    {
		...
		public AbstractDownloader(Uri baseUri, int maxTaskCount, string module = "")
        {
        ...
        protected virtual IEnumerator DoDownloadManifest(string relativePath, IProgressPromise<Progress, BundleManifest> promise)
        {
            ...
            try
            {
				...
				FileInfo file = new FileInfo(GetFullSavePath(relativePath));
                if (file.Exists)
                {
					FileInfo bakFile = new FileInfo(GetFullSavePath(relativePath + ".bak"));
                    if (bakFile.Exists)
                        bakFile.Delete();
